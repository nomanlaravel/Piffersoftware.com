$(function () {
    // Submit Holiday Form
    $(document).on("submit", "#addHolidayForm", async function (e) {
        e.preventDefault();
        var form = $(this);
    ...
    $(document).on("click", ".update-Holiday-status", function () {
        var id = $(this).data('id');
        var action = $(this).data('action');
        var modal = $('#Holiday_status_modal');
        var btn = $('#confirm_status_btn');

        $('#status_Holiday_id').val(id);
        $('#status_action').val(action);
        $('#status_remarks').val('');

        if (action === 'approved') {
            $('#status_label_text').text('Mark this Holiday as APPROVED?').addClass('text-success').removeClass('text-danger');
            btn.addClass('btn-approve-decision').removeClass('btn-reject-decision');
        } else {
            $('#status_label_text').text('Mark this Holiday as REJECTED?').addClass('text-danger').removeClass('text-success');
    ...
    $(document).on("submit", "#updateHolidayStatusForm", async function (e) {
        e.preventDefault();
        var form = $(this);
        var btn = $('#confirm_status_btn');
        ...
        try {
            ...
            if (response.ok) {
                ...
                $('#Holiday_status_modal').modal('hide');
                if (window.refreshTable) refreshTable();
            } else {
